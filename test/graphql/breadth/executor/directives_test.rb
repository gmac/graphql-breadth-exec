...
class GraphQL::Breadth::Executor::DirectivesTest < Minitest::Test
  class RecordingDirectiveResolver < GraphQL::Breadth::DirectiveResolver
    attr_reader :calls
  ...
  class ReplaceDirectiveResolver < GraphQL::Breadth::DirectiveResolver
    def initialize
      super(wraps: true)
    end
  ...
  BASE_RESOLVERS = {
    "Widget" => {
      ...
      "nested" => GraphQL::Breadth::HashKeyResolver.new("nested"),
    },
    "Query" => {
  ...
  def test_resolves_operation_and_field_directives
    ...
    result = GraphQL::Breadth::Executor.new(DIRECTIVE_SCHEMA, document, resolvers: resolvers, root_object: SOURCE).result
  ...
  def test_builds_directives_from_schema_definitions
    ...
    executor = GraphQL::Breadth::Executor.new(DIRECTIVE_SCHEMA, document, resolvers: BASE_RESOLVERS, root_object: SOURCE)
  ...
  def test_invalid_directive_arguments_are_reported_then_raised_during_execution
    ...
    reported = assert_error_reported(GraphQL::Breadth::InputValidationErrorSet) do
      result = GraphQL::Breadth::Executor.new(
        DIRECTIVE_SCHEMA,
