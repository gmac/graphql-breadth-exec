...
class GraphQL::Breadth::Executor::AuthorizationTest < Minitest::Test
  ...
  class RecordingAuthorization < GraphQL::Breadth::Authorization
    class << self
    ...
    def unauthorized_object_indices(exec_scope, context)
      ...
      self.class.object_invalidations.fetch(exec_scope.path, GraphQL::Breadth::EMPTY_OBJECT)
    end
  end
  ...
  def test_invalidates_unauthorized_objects_before_scope_execution
    RecordingAuthorization.object_invalidations = {
      ["widgets"] => {
        1 => GraphQL::Breadth::ExecutionError.new("Object hidden"),
      },
    }
