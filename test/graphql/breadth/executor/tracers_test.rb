...
class GraphQL::Breadth::Executor::TracersTest < Minitest::Test
  class BoomResolver < GraphQL::Breadth::FieldResolver
    def resolve(_exec_field, _context)
      raise StandardError, "boom"
    end
  end

  class ExtensionTracer < GraphQL::Breadth::Tracer
  ...
  class TestTracer < GraphQL::Breadth::Tracer
  ...
  def test_calls_start_finish_and_execute_hooks
    result = breadth(@document, @source, tracers: [@tracer])
    ...
    assert @tracer.start_calls.all? { _1.is_a?(GraphQL::Breadth::Executor) }
    assert @tracer.finish_calls.all? { _1.is_a?(GraphQL::Breadth::Executor) }
  ...
  def test_calls_before_format_errors
    breadth(@document, @source, tracers: [@tracer])

    assert_equal 1, @tracer.before_format_errors_calls.size
    assert @tracer.before_format_errors_calls.all? { _1.is_a?(GraphQL::Breadth::Executor) }
  end

  def test_tracers_can_install_response_extensions
    result = breadth(@document, @source, tracers: [ExtensionTracer.new])
