...
class GraphQL::Breadth::Executor::ConditionalsTest < Minitest::Test
  SOURCE = {
    "products" => {
      "nodes" => [
  ...
  def test_follows_skip_directive_omissions
    ...
    assert_equal SKIPPED_SOURCE, breadth(document, SOURCE).dig("data")
  end
  ...
  def test_follows_skip_directive_inclusions
    ...
    assert_equal SOURCE, breadth(document, SOURCE).dig("data")
  end
  ...
  def test_follows_skip_directives_with_string_variable
    ...
    assert_equal SKIPPED_SOURCE, breadth(document, SOURCE, variables: { "skip" => true }).dig("data")
  end
  ...
  def test_follows_include_directives_with_string_variable
    ...
    assert_equal SKIPPED_SOURCE, breadth(document, SOURCE, variables: { "include" => false }).dig("data")
  end
