...
class GraphQL::Breadth::Executor::ErrorsTest < Minitest::Test
  ...
  BUILD_ERROR_SCHEMA.rescue_from(GraphQL::Breadth::InvalidListResultError) do
    GraphQL::ExecutionError.new("Handled build error")
  end

  BUILD_ERROR_RESOLVERS = {
    "Query" => {
      "badList" => GraphQL::Breadth::HashKeyResolver.new("badList"),
      "ok" => GraphQL::Breadth::HashKeyResolver.new("ok"),
    },
  }.freeze
  ...
  class ResultCoercionQuery < GraphQL::Schema::Object
    ...
    field :nullable_nil, NilResultScalar, null: true do |field|
      field.breadth_resolver = GraphQL::Breadth::HashKeyResolver.new("nullableNil")
    end

    field :required_nil, NilResultScalar, null: false do |field|
      field.breadth_resolver = GraphQL::Breadth::HashKeyResolver.new("requiredNil")
    end

    field :nullable_error, ErrorResultScalar, null: true do |field|
      field.breadth_resolver = GraphQL::Breadth::HashKeyResolver.new("nullableError")
    end
  end
  ...
  def test_nullable_positional_error_adds_path
    ...
    source = {
      "products" => {
        "nodes" => [
          { "maybe" => "okay!" },
          { "maybe" => nil },
          { "maybe" => GraphQL::Breadth::ExecutionError.new("Not okay!") },
        ],
      },
    }
    ...
    assert_equal expected, breadth(document, source)
  end
  ...
  def test_handled_build_result_error_stays_field_local
    ...
    executor = GraphQL::Breadth::Executor.new(BUILD_ERROR_SCHEMA, document, resolvers: BUILD_ERROR_RESOLVERS, root_object: source)
  ...
  def test_non_null_positional_error_adds_path_and_propagates
    ...
    source = {
      "products" => {
        "nodes" => [
          { "must" => "okay!" },
          { "must" => GraphQL::Breadth::ExecutionError.new("Not okay!") },
        ],
      },
    }
