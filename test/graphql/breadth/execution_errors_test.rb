...
class GraphQL::Breadth::ExecutionErrorsTest < Minitest::Test
  def setup
  ...
  def test_execution_error_from_returns_same_unreported_error_instance
    err = GraphQL::Breadth::ExecutionError.from(GraphQL::Breadth::UNREPORTED_ERROR, exec_field: @exec_field)

    assert_same GraphQL::Breadth::UNREPORTED_ERROR, err
  ...
  def test_execution_error_from_wraps_graphql_execution_error
    ...
    err = GraphQL::Breadth::ExecutionError.from(gem_err, exec_field: @exec_field)
  ...
  def test_execution_error_from_dups_execution_error_when_exec_field_differs
    original = GraphQL::Breadth::ExecutionError.new("Whoops", exec_field: @other_exec_field)
    err = GraphQL::Breadth::ExecutionError.from(original, exec_field: @exec_field)
  ...
  def test_execution_error_from_preserves_execution_error_set_type
    original = GraphQL::Breadth::ExecutionErrorSet.new("Combined errors", exec_field: @other_exec_field)
    original.add_error("Error 1")
    original.add_error("Error 2")
    err = GraphQL::Breadth::ExecutionError.from(original, exec_field: @exec_field)

    assert_instance_of GraphQL::Breadth::ExecutionErrorSet, err
    assert_equal 2, err.errors.size
    assert_same @exec_field, err.exec_field
  end

  def test_input_error_keeps_input_path_when_wrapped
    original = GraphQL::Breadth::InputError.new("Invalid input", path: ["field", "input"])
    err = GraphQL::Breadth::ExecutionError.from(original, exec_field: @exec_field)

    assert_instance_of GraphQL::Breadth::InputError, err
  ...
  def test_result_count_mismatch_error_message
    ...
    err = GraphQL::Breadth::ResultCountMismatchError.new(exec_field:, expected_count: 1, actual_count: 3)
