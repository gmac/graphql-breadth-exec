...
class GraphQL::BreadthExec::Executor::ExecutionFieldTest < Minitest::Test
  ...
  def test_resolve_all_maps_graphql_execution_error_to_object_set
    ...
    assert_instance_of GraphQL::BreadthExec::ExecutionError, result.first
  ...
  def build_field(document_string, variables: {}, objects: [{}])
    ...
    executor = GraphQL::BreadthExec::Executor.new(TEST_SCHEMA, document, variables: variables)
    ...
    exec_scope = GraphQL::BreadthExec::Executor::ExecutionScope.new(
      executor: executor,
    ...
    GraphQL::BreadthExec::Executor::ExecutionField.new(
      node.alias || node.name,
      nodes: [node],
      scope: exec_scope,
      definition: definition,
      resolver: GraphQL::BreadthExec::HashKeyResolver.new(node.name),
    )
  end
end
